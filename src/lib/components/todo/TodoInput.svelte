...
<div class="flex items-center justify-between rounded-md border-2 border-gray-one px-5 py-4">
	<div class="flex w-full max-w-lg items-center justify-start">
		<label for={`${todo.id}--checkbox`} class="sr-only">Complete task</label>
		<input
			id={`${todo.id}--checkbox`}
			type="checkbox"
		...
		/>
		<label for={`${todo.id}--text`} class="sr-only">Edit task</label>
		<input
			id={`${todo.id}--text`}
			type="text"
			...
			class="font-noraml ml-5 flex-1 text-ellipsis rounded-md border-x-0 border-b border-t-0 border-dashed border-b-gray-two bg-cream-four px-0 pb-1 text-base text-gray-three placeholder:text-gray-two focus:border-gray-three focus:outline-none focus:ring-0"
		/>
		<span
